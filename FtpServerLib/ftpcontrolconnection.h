...
private:
    ...
    // The current directory (i.e. CD) of the FTP server. Used for relative paths.
    QString         m_currentDirectory;

    // Some commands depend on the previous command (for example RNFR/RNTO, REST/STOR, etc.).
    ...
    bool            m_encryptDataConnection;

    DataConnection  *m_dataConnection;

    // Flag whether the client is allowed only read-only access (can download, but not upload/modify).
    bool            m_readOnly;
};
